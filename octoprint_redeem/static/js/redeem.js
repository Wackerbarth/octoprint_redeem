$(function() {
  function RedeemViewModel(parameters) {
    ...
    self.uploadElement.fileupload({
      ...
      add: function(e, data) {
        ...
        self.placeholderDescription(
            'Imported from ' + self.fileName() + ' on ' +
            formatDate(new Date().getTime() / 1000));

        self.uploadButton.unbind('click');
        self.uploadButton.on('click', function() {
          var form = {allowOverwrite: self.profileAllowOverwrite()};
    ...
    self.removeProfile = function(data) {
      ...
      $.ajax({
        ...
        data: JSON.stringify({command: 'delete_profile', key: data.key}),
        success: function() {
          self.requestData();
        }
      });
    ...
    self.makeProfileDefault = function(data) {
      ...
      $.ajax({
        ...
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify(
            {command: 'use_profile', default: true, key: data.key}),
        success: function() {
          self.requestData();
        }
      });
    ...
    self.showEditLocalDialog = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'get_profile', key: 'local.cfg'}),
    ...
    self.showViewProfileDialog = function(filename) {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'get_profile', key: filename}),
        success: function(data) {
          $('#settings_plugin_redeem_textarea_viewer').val(data['data']);
        }
      });
      $('#settings_plugin_redeem_config_viewer')
          .text('Viewer for config file ' + filename);
    ...
    self.restartRedeem = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'restart_redeem'}),
        success: function() {
          options = {
    ...
    self.upgradeRedeem = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'redeem_upgrade_current_branch'}),
        success: function() {
          options = {
    ...
    self.checkForUpdates = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'redeem_is_current_branch_upgradable'}),
        success: function(ret) {
          ...
          if (data['is_upgradable']) {
            options = {
              ...
              text: 'Current version: ' + data['local_version'] +
                  '<br>Available upgrade: ' + data['remote_version'],
              type: 'info',
              hide: false,
              confirm: {
                confirm: true,
    ...
    self.requestData = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({'command': 'get_list_of_profiles'}),
    ...
    self.resetThermistorAlarm = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'reset_thermistor_alarm'}),
        success: function() {
          self.removePopups();
        }
      });
    ...
    self.resetEndstopAlarm = function() {
      $.ajax({
        ...
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
        data: JSON.stringify({command: 'reset_endstop_alarm'}),
        success: function() {
    ...
    self.uploadAndViewBedData = function(data_type, probe_data, replicape_key) {
      $.ajax({
        url: 'http://review.thing-printer.com/api/index.php/post_probe_data/' +
            replicape_key,
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json; charset=UTF-8',
              ...
              buttons: [{
                ...
                click: function() {
                  self.uploadAndViewBedData(
                      messageData.probe_type, messageData.probe_data,
  ...
  OCTOPRINT_VIEWMODELS.push([
    RedeemViewModel, ['loginStateViewModel', 'settingsViewModel'],
