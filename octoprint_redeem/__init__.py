...
class RedeemPlugin(octoprint.plugin.TemplatePlugin, octoprint.plugin.SettingsPlugin,
  ...
  #~~ AssetPlugin API
  def get_assets(self):
    return {"js": ["js/redeem.js"], "less": ["less/redeem.less"], "css": ["css/redeem.css"]}
      ...
      for printer in printers:
        ...
        profiles[printer] = {
            ...
            "refs": {
                "resource":
                    flask.url_for(
                        "plugin.redeem.download_profile", filename=printer, _external=True),
                "download":
                    flask.url_for("index", _external=True) + "plugin/redeem/download/" + printer
            }
        }
  ...
  def import_redeem_profile(self):
    ...
    if input_upload_name in flask.request.values and input_upload_path in flask.request.values:
      ...
      try:
        ...
        return flask.make_response(
            "Something went wrong while converting imported profile: {message}".format(
                message=str(e)),
            500)
    ...
    try:
      ...
      return flask.make_response(
          "Error renaming file. Perhaps wrong permissions".format(**locals()), 409)

    result = dict(
        resource=flask.url_for(
            "api.slicingGetSlicerProfile", slicer="cura", name=profile_name, _external=True),
  ...
  def custom_action_handler(self, comm, line, action, *args, **kwargs):
    ...
    if action[:6] == "alarm_":
      self._plugin_manager.send_plugin_message("redeem", dict(
          type=action, data={"message": message}))
    elif action == "filament_sensor":
      timeUTC = int(time.time())
      self._plugin_manager.send_plugin_message(
          "redeem", dict(type=action, data={
              "message": message,
              "time": str(timeUTC)
          }))
